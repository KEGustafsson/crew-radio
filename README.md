...
everything the short account below leaves out: creating the release keystore, the
environment variables the build reads, the workflow secrets, and what a push to `main`
publishes.
...
can use it. The developer notes are in [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), the build
and release procedures in [docs/BUILDING.md](docs/BUILDING.md), and the diagrams are draw.io
files under [docs/diagrams](docs/diagrams), generated by `make_diagrams.py` there.
