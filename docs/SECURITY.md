...
| Taking a phone's outgoing audio | Anyone on the same WLAN | Audio goes unicast to the peers heard from, and to the group and broadcast only while none is known — so a table filled with a stranger's addresses would take every frame while the phone still looked healthy on the crew's roster and still heard them. Addresses are therefore learned only from packets that opened with the channel key (`Transport.confirmPeer`), never from the datagram. |
