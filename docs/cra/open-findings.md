...
| Source-address ownership | The AEAD authenticates the packet, not the address it arrived from. So a replay of a captured frame, sent from another address and winning the race against the genuine copy, can still enter the LAN peer table — a much higher bar than the garbage that used to work, but not closed. Proving an address holds the key needs a challenge-response, which is a handshake this design deliberately does not have. **Mitigated meanwhile:** the first frames of every talk burst keep the group and broadcast copies whatever the table holds, so a poisoned table costs audio quality rather than all audio, and the fault is audible instead of silent. |
